...
func (s *FunctionSuite) TestFnMsgData() {
	type txMsgDataCase struct {
		Name   string
		Length uint64
		Res    []byte
		Err    error
	...
	testcases := []txMsgDataCase{
		{"address with length 1", 1, res, nil},
		{"address with length 10", 10, res, nil},
		{"address with length 0", 0, res, nil},
	}
